		...
		>
			...
			{pages.map((p, i) =>
				p === null ? (
					<span
						key={`ellipsis-${i}`}
						className="flex items-center justify-center h-9 w-6 text-ink-3 text-sm select-none"
					>
						…
					</span>
				) : p === meta.page ? (
					<span key={p} className={activeLink} aria-current="page">
						{p}
					</span>
				) : (
					<Link key={p} href={href(p)} className={normalLink}>
						{p}
					</Link>
				),
			)}
