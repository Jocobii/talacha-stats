 ...
 */
export function buildTickerItems(leagues: LeagueInfo[], snapshots: LeagueSnapshot[]): TickerItem[] {
 ...
 */
export function buildNarrativeLine(league: LeagueInfo, snapshot: LeagueSnapshot): string {
	const parts: string[] = [];
	...
	if (snapshot.topScorer) {
		const name = snapshot.topScorer.alias
			? `«${titleCase(snapshot.topScorer.alias)}»`
			: titleCase(snapshot.topScorer.fullName);
		parts.push(`${name} va por el título de goleo con ${snapshot.topScorer.goals} goles.`);
	}
