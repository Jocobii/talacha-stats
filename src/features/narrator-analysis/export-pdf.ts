...
export async function buildNarratorPdf(analysis: NarratorAnalysis): Promise<Buffer> {
	return new Promise((resolve, reject) => {
		...
		doc
			...
			.fontSize(8)
			.font("Helvetica")
			.text(`${analysis.league.name.toUpperCase()} · ${analysis.league.season}`, 40, 50, {
				width: W,
				align: "center",
			});
		doc
		...
		doc.fill(C.red).text(`${b.team.name}: ${bWinPct}%`, { align: "right", width: W / 3 });
		doc.moveDown(0.8);
		...
		if (a.currentStreak && a.currentStreak.count >= 2) {
			...
			const col =
				a.currentStreak.type === "W" ? C.green : a.currentStreak.type === "L" ? C.red : C.gray;
		...
		if (a.attackRank !== null && a.totalTeams > 2) {
			...
			const rankLabel =
				a.attackRank <= Math.ceil(t / 3)
					? "Ataque fuerte"
					: a.attackRank <= Math.ceil((t * 2) / 3)
						? "Ataque regular"
						: "Ataque flojo";
			const defLabel =
				a.defenseRank !== null
					? a.defenseRank <= Math.ceil(t / 3)
						? "  Defensa solida"
						: a.defenseRank <= Math.ceil((t * 2) / 3)
							? "  Defensa regular"
							: "  Defensa debil"
					: "";
		...
		if (b.currentStreak && b.currentStreak.count >= 2) {
			...
			const col =
				b.currentStreak.type === "W" ? C.green : b.currentStreak.type === "L" ? C.red : C.gray;
		...
		if (b.attackRank !== null && b.totalTeams > 2) {
			...
			const rankLabel =
				b.attackRank <= Math.ceil(t / 3)
					? "Ataque fuerte"
					: b.attackRank <= Math.ceil((t * 2) / 3)
						? "Ataque regular"
						: "Ataque flojo";
			const defLabel =
				b.defenseRank !== null
					? b.defenseRank <= Math.ceil(t / 3)
						? "  Defensa solida"
						: b.defenseRank <= Math.ceil((t * 2) / 3)
							? "  Defensa regular"
							: "  Defensa debil"
					: "";
		...
		if (threatsA.length > 0 || threatsB.length > 0) {
			...
			if (threatsA.length > 0) {
				...
				for (const p of threatsA) {
					const dangerLabel =
						p.dangerRating === "ALTO" ? "[ALTO]" : p.dangerRating === "MEDIO" ? "[MED]" : "";
					ytA = teamLine(
		...
		function drawRosterRow(p: (typeof rA)[number] | undefined, x: number, y: number) {
			if (!p) return;
			const cells = [
