...
export function buildNarratorPngElement(analysis: NarratorAnalysis): React.ReactElement {
	...
	return (
		...
		>
			...
			>
				...
				>
					<div style={{ display: "flex", flexDirection: "column", flex: 1 }}>
						<span style={{ fontSize: 26, fontWeight: 900, color: "#93c5fd" }}>{a.team.name}</span>
						{a.position != null && (
							<span style={{ fontSize: 11, color: C.gray }}>{a.position}° en tabla</span>
						)}
					</div>
					<span
						style={{
					...
					>
						<span style={{ fontSize: 26, fontWeight: 900, color: "#fca5a5" }}>{b.team.name}</span>
						{b.position != null && (
							<span style={{ fontSize: 11, color: C.gray }}>{b.position}° en tabla</span>
						)}
					</div>
				</div>
			</div>
			...
			<Section>
				...
				>
					<span style={{ fontSize: 11, color: C.blue, fontWeight: 600 }}>{a.team.name}</span>
					<span style={{ fontSize: 11, color: C.gray }}>Empate</span>
					<span style={{ fontSize: 11, color: C.red, fontWeight: 600 }}>{b.team.name}</span>
				</div>
			</Section>
			...
			{analysis.matchPrediction.hasData &&
				(() => {
					...
					const totalLabelMap = {
						...
						festival: "Festival de goles",
					};
					const totalColor = { cerrado: "#1d4ed8", abierto: "#b45309", festival: "#dc2626" }[
						pred.totalLabel
					];
					const edgeLabel = (e: string) =>
						e === "A" ? a.team.name : e === "B" ? b.team.name : "Equilibrado";
					return (
						<Section>
							...
							>
								<div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
									<span style={{ fontSize: 32, fontWeight: 900, color: C.blueDk }}>
										{pred.likelyScoreA}
									</span>
								...
								<div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
									<span style={{ fontSize: 32, fontWeight: 900, color: C.redDk }}>
										{pred.likelyScoreB}
									</span>
							...
							<div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
								...
								>
									<span style={{ fontSize: 18, fontWeight: 900, color: C.blueDk }}>
										{pred.expectedGoalsA}
									</span>
								...
								>
									...
									<span style={{ fontSize: 9, color: totalColor, fontWeight: 700 }}>
										{totalLabelMap[pred.totalLabel]}
									</span>
								</div>
								<div
									style={{
								...
								>
									<span style={{ fontSize: 18, fontWeight: 900, color: C.redDk }}>
										{pred.expectedGoalsB}
									</span>
							...
							<div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
								{[
									[
										...
										pred.bothTeamsToScore ? "Probable que si" : "No garantizado",
									],
			...
			{(analysis.positionSimulator.teamA.currentPosition !== null ||
				analysis.positionSimulator.teamB.currentPosition !== null) &&
				(() => {
								...
								{(
									...
									>
										...
										<span style={{ fontSize: 10, color: C.gray }}>
											Ahora: {pos(s.currentPosition)} · {s.currentPoints} pts
										</span>
										{[
											["Si gana", s.ifWin],
											["Si empata", s.ifDraw],
								...
								{team.topScoringThreats.length === 0 ? (
									...
									team.topScoringThreats.map((p, i) => (
										...
										>
											...
											<div style={{ display: "flex", flexDirection: "column", flex: 1 }}>
												<span style={{ fontSize: 11, fontWeight: 600, color: C.dark }}>
													{displayName(p)}
												</span>
			...
			{h2h.total > 0 && (
				<Section>
					...
					>
						...
						>
							<span style={{ fontSize: 36, fontWeight: 900, color: C.blueDk }}>{h2h.aWins}</span>
						...
						>
							<span style={{ fontSize: 28, fontWeight: 900, color: C.lgray }}>{h2h.draws}</span>
						...
						>
							<span style={{ fontSize: 36, fontWeight: 900, color: C.redDk }}>{h2h.bWins}</span>
					...
					{h2h.lastMatch && (
						...
						>
							Ultimo: {h2h.lastMatch.aGoals}–{h2h.lastMatch.bGoals} ({h2h.lastMatch.result}) ·{" "}
							{h2h.lastMatch.date}
						</span>
					)}
				</Section>
...
function TeamCard({ team, color }: { team: NarratorAnalysis["teamA"]; color: "blue" | "red" }) {
	...
	return (
		...
		>
			<span style={{ fontSize: 15, fontWeight: 900, color: accent }}>{team.team.name}</span>
			...
			<div style={{ display: "flex", gap: 6 }}>
				{[
					...
					>
						<span style={{ fontSize: 16, fontWeight: 900, color: C.dark }}>{s.v}</span>
			...
			<div style={{ display: "flex", gap: 10, fontSize: 11, color: C.gray }}>
				...
				<span>
					Dif{" "}
					<strong style={{ color: team.goalDiff >= 0 ? C.green : C.red }}>
						{team.goalDiff >= 0 ? "+" : ""}
						{team.goalDiff}
					</strong>
				</span>
			...
			{last5.length > 0 && (
				<div style={{ display: "flex", gap: 4, alignItems: "center" }}>
					<span style={{ fontSize: 10, color: C.gray, marginRight: 2 }}>Últ. {last5.length}:</span>
					{last5.map((r, i) => (
						<div
							key={i}
							style={{
								...
								height: 20,
								borderRadius: "50%",
								background: r === "W" ? C.green : r === "D" ? C.gray : C.red,
								alignItems: "center",
								justifyContent: "center",
								color: C.white,
								fontSize: 9,
				...
				<div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
					{team.attackRank !== null &&
						(() => {
							...
							const [label, bg, color] =
								r <= Math.ceil(t / 3)
									? ["Ataque fuerte", "#dcfce7", "#15803d"]
									: r <= Math.ceil((t * 2) / 3)
										? ["Ataque regular", "#fef9c3", "#92400e"]
					...
					{team.defenseRank !== null &&
						(() => {
							...
							const [label, bg, color] =
								r <= Math.ceil(t / 3)
									? ["Defensa solida", "#dcfce7", "#15803d"]
									: r <= Math.ceil((t * 2) / 3)
										? ["Defensa regular", "#fef9c3", "#92400e"]
			...
			{team.topScorer && (
				<div style={{ display: "flex", gap: 6, fontSize: 11, alignItems: "center" }}>
					<span>⚽</span>
					<span style={{ color: C.dark, fontWeight: 600 }}>{displayName(team.topScorer)}</span>
					<span style={{ color: C.gray, marginLeft: "auto" }}>{team.topScorer.goals} goles</span>
				</div>
			)}
			...
			{team.topAssist && team.topAssist.playerId !== team.topScorer?.playerId && (
				<div style={{ display: "flex", gap: 6, fontSize: 11, alignItems: "center" }}>
					<span>🎯</span>
					<span style={{ color: C.dark, fontWeight: 600 }}>{displayName(team.topAssist)}</span>
					<span style={{ color: C.gray, marginLeft: "auto" }}>{team.topAssist.assists} asist.</span>
				</div>
			)}
