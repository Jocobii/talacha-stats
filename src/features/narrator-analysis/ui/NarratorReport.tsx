/**
 * features/narrator-analysis/ui/NarratorReport.tsx
 *
 * Reporte de análisis reutilizable (mobile-first). Consume un `NarratorAnalysis`
 * y opcionalmente una barra de `actions` (compartir, exportar, nuevo análisis).
 * Lo usan tanto el flujo BD (`/analysis`) como el flujo Excel público, sin
 * duplicar el render. Componente tonto: cero red, cero estado.
 *
 * Las secciones que dependen de plantel/H2H (RosterTable, ScoringThreats, H2H)
 * se ocultan solas cuando no hay datos — clave para el flujo Excel, que solo
 * trae la tabla de posiciones.
 */
...
export function NarratorReport({
	analysis,
	actions,
}: {
	...
	return (
		<div className="space-y-4">
			{actions && <div className="flex justify-end gap-2 flex-wrap">{actions}</div>}
			...
			<div className="bg-pitch border border-line rounded-2xl p-5 text-center">
				<p className="text-xs text-ink-3 uppercase tracking-widest mb-1">
					...
					{analysis.league.season ? ` · ${analysis.league.season}` : ""}
				</p>
				{analysis.lastMatchday != null && (
					<p className="text-xs text-ink-3 mb-3">
						Datos hasta la jornada{" "}
						<span className="font-semibold text-ink-2">{analysis.lastMatchday}</span>
					</p>
				)}
				<div className="flex items-center justify-center gap-3">
					<div className="text-right flex-1 min-w-0">
						<p className="text-base sm:text-xl font-display font-black text-blue-400 leading-tight break-words">
							{teamA.team.name}
						</p>
						{teamA.position && (
							<p className="text-xs text-ink-3 mt-0.5">{teamA.position}° en tabla</p>
						)}
					</div>
					<div className="font-display font-black text-xl text-ink-3 shrink-0 px-1">VS</div>
					<div className="text-left flex-1 min-w-0">
						<p className="text-base sm:text-xl font-display font-black text-red-400 leading-tight break-words">
							{teamB.team.name}
						</p>
						{teamB.position && (
							<p className="text-xs text-ink-3 mt-0.5">{teamB.position}° en tabla</p>
						)}
					</div>
				</div>
			</div>
		...
		<div className={`bg-surface-2 border border-line border-t-2 ${topBorder} rounded-2xl p-5`}>
			...
			<div className="flex gap-3 mb-3 text-sm flex-wrap">
				<span className="text-ink-3">
					GF <strong className="text-ink">{team.goalsFor}</strong>
				</span>
				<span className="text-line">|</span>
				<span className="text-ink-3">
					GC <strong className="text-ink">{team.goalsAgainst}</strong>
				</span>
				<span className="text-line">|</span>
				<span className="text-ink-3">
					Dif{" "}
				...
				<span className="text-line">|</span>
				<span className="text-ink-3">
					Prom <strong className="text-ink">{team.avgGoalsFor}</strong>
				</span>
			</div>
			...
			{(team.attackRank !== null || team.defenseRank !== null) && team.totalTeams > 2 && (
				<div className="flex gap-2 mb-3 flex-wrap">
					{team.attackRank !== null && (
						<RankBadge kind="attack" rank={team.attackRank} total={team.totalTeams} />
					)}
					{team.defenseRank !== null && (
						<RankBadge kind="defense" rank={team.defenseRank} total={team.totalTeams} />
					)}
				</div>
			)}
...
function RankBadge({
	...
	const strong = rank <= Math.ceil(total / 3);
	const mid = rank <= Math.ceil((total * 2) / 3);
	const cls = strong
		? "bg-green-950 text-green-400 border-green-900"
		: mid
			? "bg-yellow-950 text-yellow-400 border-yellow-900"
			: "bg-red-950 text-red-400 border-red-900";
	const word = kind === "attack" ? "Ataque" : "Defensa";
	const quality = strong ? "fuerte" : mid ? "regular" : "flojo";
	const icon = kind === "attack" ? "⚔️" : "🛡️";
	return (
		<span className={`text-xs px-2 py-0.5 rounded-full font-medium border ${cls}`}>
			{icon} {word} {quality} ({rank}/{total})
		</span>
	);
}
			...
			{h2h.lastMatch && (
				<p className="text-center text-xs text-ink-3 mt-4 border-t border-line pt-3">
					...
					<strong className="text-ink">
						{h2h.lastMatch.aGoals}–{h2h.lastMatch.bGoals}
					</strong>{" "}
...
function RosterTable({ team, color }: { team: TeamAnalysis; color: "blue" | "red" }) {
	...
	const accent =
		color === "blue" ? "text-blue-400 border-t-blue-500" : "text-red-400 border-t-red-500";
	...
	return (
		...
		>
			<div className="px-4 py-3 border-b border-line">
				<h3
					className={`font-display font-black text-lg uppercase tracking-wide ${color === "blue" ? "text-blue-400" : "text-red-400"}`}
				>
					{team.team.name}
				</h3>
			</div>
				...
				<table className="w-full text-xs">
					...
					<tbody className="divide-y divide-line">
						{team.roster.map((p) => (
							<tr
								key={p.playerId}
								className={`hover:bg-pitch transition ${p.contributions === 0 ? "opacity-40" : ""}`}
							>
		...
		<div className="bg-surface-2 border border-line rounded-2xl p-5 space-y-5">
			...
			<div className="grid grid-cols-3 gap-3 text-center">
				...
				<div className="bg-pitch border border-line rounded-xl p-3 flex flex-col items-center justify-center gap-1">
					...
					<span className={`text-[9px] font-semibold px-1.5 py-0.5 rounded-full ${tl.cls}`}>
						{tl.text}
					</span>
				</div>
			...
			<div className="space-y-2 border-t border-line pt-4">
				...
				<div className="flex items-center justify-between text-sm">
					...
					<span className="font-semibold text-ink">
						{edgeLabel(pred.offensiveEdge, nameA, nameB)}
					</span>
				</div>
				<div className="flex items-center justify-between text-sm">
					<span className="text-ink-3">Ventaja defensiva</span>
					<span className="font-semibold text-ink">
						{edgeLabel(pred.defensiveEdge, nameA, nameB)}
					</span>
				</div>
			</div>
		</div>
...
function PositionSimulatorCard({
	sim,
	nameA,
	nameB,
}: {
		...
		<div className="bg-surface-2 border border-line rounded-2xl p-5">
			...
			<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
				{(
					...
					<div
						key={name}
						className={`rounded-xl border p-4 ${color === "blue" ? "bg-blue-950 border-blue-900" : "bg-red-950 border-red-900"}`}
					>
						<p
							className={`font-display font-black text-lg uppercase mb-1 ${color === "blue" ? "text-blue-400" : "text-red-400"}`}
						>
							{name}
						</p>
						<p className="text-xs text-ink-3 mb-3">
							Posición: <strong className="text-ink">{s.currentPosition ?? "—"}°</strong> ·{" "}
							{s.currentPoints} pts
						</p>
			...
			<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
				{(
					...
					<div key={team.team.id}>
						<p
							className={`text-xs font-bold uppercase tracking-widest mb-3 ${color === "blue" ? "text-blue-400" : "text-red-400"}`}
						>
							{team.team.name}
						</p>
                      ...
                      ${i === 0 ? (color === "blue" ? "bg-blue-500" : "bg-red-500") : "bg-surface border border-line text-ink-3"}`}
										...
										<div className="flex-1 min-w-0">
											...
											<p className="text-xs text-ink-3">
												{p.goals} goles · {p.goalsPerMatch} x partido
											</p>
										</div>
										<DangerBadge rating={p.dangerRating} goals={p.goals} />
									</div>
