	...
	return (
		<div className="flex flex-col gap-5">
			<div className="bg-surface rounded-2xl border border-line shadow-sm p-5">
				<h2 className="text-lg font-bold text-ink mb-1">Vista previa del Excel</h2>
				...
				<div className="grid grid-cols-2 gap-3">
					{stats.map((s) => (
						<div
							key={s.label}
							className={["rounded-xl border p-3 flex items-center gap-3", s.color].join(" ")}
						>
							<span className="text-2xl shrink-0">{s.icon}</span>
							<div>
								<p className="text-2xl font-black leading-none">{s.value}</p>
								<p className="text-[12px] font-medium leading-snug mt-0.5">{s.label}</p>
							</div>
						</div>
					))}
				</div>
			...
			{warnings.length > 0 && (
				<div className="rounded-xl border border-orange-200 bg-orange-50 p-4 flex flex-col gap-1.5">
					<p className="text-xs font-semibold text-orange-700 uppercase tracking-wide">Avisos</p>
					{warnings.map((w, i) => (
