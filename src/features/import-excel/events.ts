...
export async function generateEventPreview(
  ...
  const playerMatches: PlayerMatch[] = playerResolutions.map((res) => {
    ...
    if (candidates.length > 1) {
      warnings.push(`"${res.rawName}" tiene ${candidates.length} candidatos — seleccionar manualmente.`);
    } else {
...
export async function confirmEventImport(
    ...
    allTeamNamesRaw.length > 0
      ? await db.query.teams.findMany({
          where: and(
            ...
            inArray(teams.name, allTeamNamesRaw),
          ),
  ...
  const matchKeyToId = new Map<string, string>(
    existingMatches.map((m) => [
      matchKey(m.matchday ?? 0, m.homeTeamId, m.awayTeamId),
      m.id,
    ]),
  );
...
function normalizeTipo(tipo: string): EventType {
  const map: Record<string, EventType> = {
    gol: "goal", goal: "goal",
    "gol propio": "own_goal", autogol: "own_goal", own_goal: "own_goal",
    asistencia: "assist", assist: "assist",
    amarilla: "yellow_card", yellow: "yellow_card", yellow_card: "yellow_card",
    roja: "red_card", red: "red_card", red_card: "red_card",
    mvp: "mvp",
  };
