 ...
 * POST /api/auth/login
 * Body: { email, password }
 * Autentica al usuario y setea la cookie de sesión ts_session.
 */
...
export async function POST(request: Request) {
  ...
  if (!parsed.success) return apiError("Email y contraseña requeridos", 400);
  ...
  return new Response(
    JSON.stringify({ ok: true, redirect: redirectTo, user: { name: user.name, role: user.role } }),
    {
      status: 200,
      headers: {
