...
export async function POST(request: Request) {
	...
	try {
		...
		await db.transaction(async (tx) => {
			...
			const delRegs = await tx.execute(
				sql.raw(`
      ...
      `),
			...
			// ── 2. matches ───────────────────────────────────────────────────────
			const updHome = await tx.execute(
				sql.raw(`
      ...
      `),
			...
			const updAway = await tx.execute(
				sql.raw(`
      ...
      `),
			...
			updatedMatches =
				((updHome as { rowCount: number }).rowCount ?? 0) +
				((updAway as { rowCount: number }).rowCount ?? 0);

			// ── 3. match_events ──────────────────────────────────────────────────
			const updEvt = await tx.execute(
				sql.raw(`
      ...
      `),
			...
			const delStats = await tx.execute(
				sql.raw(`
      ...
      `),
			...
			const delSnap = await tx.execute(
				sql.raw(`
      ...
      `),
		...
		return apiError(
			"Error al fusionar equipos: " + (e instanceof Error ? e.message : "desconocido"),
			500,
		);
	}
}
