...
export async function POST(request: Request) {
  ...
  if (rawMapping) {
    let raw: unknown;
    try { raw = JSON.parse(rawMapping); } catch { return apiError("JSON de mapping inválido", 400); }
  ...
  try {
    ...
    return apiError(e instanceof Error ? e.message : "No se pudo parsear el archivo", 400);
  }
  ...
  if (action === "confirm") {
    ...
    if (rawRes) {
      let raw: unknown;
      try { raw = JSON.parse(rawRes); } catch { return apiError("JSON de resoluciones inválido", 400); }
    ...
    if (rawExclude) {
      ...
      try { excludeKeys = JSON.parse(rawExclude); } catch { /* ignorar */ }
      if (Array.isArray(excludeKeys)) {
        // Las keys tienen formato "g:{index}:{nombre}" o "s:{index}:{nombre}"
        const excludedIndices = new Set(
          excludeKeys.map((k: unknown) => {
            const parts = String(k).split(":");
            return parts.length >= 2 ? parseInt(parts[1], 10) : -1;
          }).filter(n => n >= 0)
        );
        if (excludedIndices.size > 0) {
          parsed = { ...parsed, rows: parsed.rows.filter((_, i) => !excludedIndices.has(i)) };
        }
      }
    }

    const result = await confirmBulkImport({ leagueId, parsed, playerResolutions });
