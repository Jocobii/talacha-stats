...
function weightedPickN(
	arr: { id: string; totalGoals: number }[],
	n: number,
): { id: string; totalGoals: number }[] {
...
function simulateMatchScore(homeStrength: number, awayStrength: number): [number, number] {
	...
	return [poissonSample(Math.max(2.0, homeExp)), poissonSample(Math.max(2.0, awayExp))];
}
...
export async function POST(request: Request) {
	...
	const { city, name, dayOfWeek, season, numTeams, numPlayersPerTeam, jornada, organizationId } =
		parsed.data;
	...
	const result = await db.transaction(async (tx) => {
		...
		const [league] = await tx
			.insert(leagues)
			.values({ name, dayOfWeek, season, city, organizationId: organizationId ?? null })
			.returning();
		...
		const teamRows = await tx
			.insert(teams)
			.values(teamNames.map((teamName) => ({ name: teamName, leagueId: league.id })))
			.returning();
		...
		for (const team of teamRows) {
			...
			const inserted = await tx
				.insert(players)
				.values(playerData.map(({ fullName, alias }) => ({ fullName, alias })))
				.returning();
		...
		for (let j = 0; j < jornada; j++) {
			...
			for (let i = 0; i + 1 < shuffled.length; i += 2) {
				...
				if (hg > ag) {
					standings[home.id].wins++;
					standings[home.id].points += 3;
					standings[away.id].losses++;
				} else if (hg < ag) {
					standings[away.id].wins++;
					standings[away.id].points += 3;
					standings[home.id].losses++;
				} else {
					standings[home.id].draws++;
					standings[home.id].points++;
					standings[away.id].draws++;
					standings[away.id].points++;
				}
			}
		}
		...
		for (const { teamId, playerIds } of teamPlayerMap) {
			...
			const regularInTeam = playerIds
				.filter((id) => !crossLeaguePlayerIds.has(id))
