...
export default function DemoView() {
	...
	const TABS: { id: Tab; label: string; icon: typeof Trophy }[] = [
		...
		{ id: "matchday", label: t("tabs.matchday"), icon: Star },
		{ id: "analisis", label: t("tabs.analysis"), icon: BarChart3 },
	];
		...
		<div className="text-ink flex flex-col flex-1 min-h-screen">
			...
			<header className="px-5 py-4 flex items-center justify-between max-w-2xl mx-auto w-full border-b border-line">
				<div>
					...
					<span className="ml-2 text-xs text-ink-3 font-medium">{t("subtitle")}</span>
				</div>
				<Link
					href="/"
					className="text-ink-3 hover:text-ink text-sm transition flex items-center gap-1"
				>
					<ArrowLeft size={16} /> {tCommon("backHome")}
				</Link>
			</header>
			...
			<div className="bg-gray-950 px-5 pt-10 pb-8">
				<div className="max-w-lg mx-auto">
					...
					<div className="bg-gray-900 rounded-2xl px-6 py-5 flex items-center justify-between">
						<div>
							<p className="text-gray-400 text-sm">{t("profile.goalsPerMatch")}</p>
						...
						<div className="text-right space-y-1">
							<p className="text-gray-300 text-sm">
								<span className="text-white font-bold">{p.global.goals}</span> {t("profile.goals")}
							</p>
							<p className="text-gray-300 text-sm">
								<span className="text-white font-bold">{p.global.matches}</span>{" "}
								{t("profile.matches")}
							</p>
							<p className="text-gray-300 text-sm">
								<span className="text-white font-bold">{p.global.leagues}</span>{" "}
								{t("profile.leagues")}
							</p>
						</div>
					</div>
				...
				<div className="max-w-lg mx-auto space-y-5">
					...
					<section>
						<h2 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">
							{t("profile.leaguesCount", { count: p.leagues.length })}
						</h2>
					...
					<div className="bg-white border border-gray-200 rounded-2xl p-4 flex items-center justify-between">
						<div>
							<p className="font-semibold text-gray-900 text-sm">{t("profile.sharePlaceholder")}</p>
