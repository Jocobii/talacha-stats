	...
	return (
		<div className="text-ink flex flex-col flex-1 min-h-screen">
			...
			<header className="px-5 py-4 flex items-center justify-between max-w-2xl mx-auto w-full border-b border-line">
				...
				<Link
					href="/"
					className="text-ink-3 hover:text-ink text-sm transition flex items-center gap-1"
				>
		...
		<div className="flex flex-col flex-1">
			...
			<div className="flex-1 bg-surface rounded-t-3xl px-4 pt-6 pb-10">
                ...
                ${p.id === "1" ? "hover:border-brand cursor-pointer transition" : ""}`}
					...
					<p className="text-center text-xs text-ink-3 pt-2">Toca al #1 para ver su perfil →</p>
				</div>
			</div>
		</div>
	);
					...
					<div className="bg-gray-900 rounded-2xl px-6 py-5 flex items-center justify-between">
						<div>
							...
							<p className="text-5xl font-black text-green-400 mt-1 leading-none">
								{p.global.gpm.toFixed(2)}
							</p>
						</div>
						<div className="text-right space-y-1">
							<p className="text-gray-300 text-sm">
								<span className="text-white font-bold">{p.global.goals}</span> goles
							</p>
							<p className="text-gray-300 text-sm">
								<span className="text-white font-bold">{p.global.matches}</span> partidos
							</p>
							<p className="text-gray-300 text-sm">
								<span className="text-white font-bold">{p.global.leagues}</span> ligas
							</p>
						</div>
					</div>
				</div>
					...
					<div className="bg-white rounded-2xl shadow-sm px-4 py-4">
						<div className="grid grid-cols-3 gap-2">
							<div className="text-center">
								<p className="text-2xl font-black text-green-600">{p.global.goals}</p>
								<p className="text-[10px] text-gray-400 mt-0.5">Goles</p>
							</div>
							<div className="text-center">
								<p className="text-2xl font-black text-gray-700">{p.global.matches}</p>
								<p className="text-[10px] text-gray-400 mt-0.5">Partidos</p>
							</div>
							<div className="text-center">
								<p className="text-2xl font-black text-purple-600">{p.global.leagues}</p>
								<p className="text-[10px] text-gray-400 mt-0.5">Ligas</p>
							</div>
						</div>
					</div>
								...
								<div key={l.name} className="bg-white rounded-2xl shadow-sm overflow-hidden">
									...
									<div className="p-4 space-y-3">
										<div className="flex items-start justify-between gap-2">
											...
											<div className="text-right shrink-0">
												<p
													className={`text-2xl font-black ${l.gpm >= 1 ? "text-green-600" : l.gpm >= 0.5 ? "text-yellow-600" : "text-gray-500"}`}
												>
													{l.gpm.toFixed(2)}
												</p>
										...
										<div className="grid grid-cols-3 gap-2">
											<div className="rounded-xl p-2.5 text-center bg-green-50">
												<p className="text-xl font-black text-green-600">{l.goals}</p>
												<p className="text-[10px] text-gray-400">Goles</p>
											</div>
											<div className="rounded-xl p-2.5 text-center bg-gray-50">
												<p className="text-xl font-black text-gray-700">{l.matches}</p>
												<p className="text-[10px] text-gray-400">PJ</p>
											</div>
											<div className="rounded-xl p-2.5 text-center bg-gray-50">
												<p className="text-xl font-black text-gray-700">{l.yellows}</p>
					...
					<div className="bg-white border border-gray-200 rounded-2xl p-4 flex items-center justify-between">
						<div>
							...
							<p className="text-xs text-gray-400 mt-0.5">
								talachastats.com/player/roberto-mendoza
							</p>
						</div>
					...
					<div className="bg-pitch border border-line rounded-2xl p-5 text-center">
						...
						<div className="flex items-center justify-center gap-3">
							<div className="text-right flex-1 min-w-0">
								<p className="text-base sm:text-xl font-display font-black text-blue-400 leading-tight">
									{a.name}
								</p>
							...
							<div className="text-left flex-1 min-w-0">
								<p className="text-base sm:text-xl font-display font-black text-red-400 leading-tight">
									{b.name}
								</p>
					...
					<div className="bg-surface-2 border border-line rounded-2xl p-5 space-y-5">
						...
						<div className="grid grid-cols-3 gap-3 text-center">
							...
							<div className="bg-pitch border border-line rounded-xl p-3 flex flex-col items-center justify-center gap-1">
								...
								<span className="text-[9px] font-semibold px-1.5 py-0.5 rounded-full bg-yellow-950 text-yellow-400 border border-yellow-900">
									Partido abierto
								</span>
							</div>
							...
							{[
								...
								>
									...
									<p className="text-xs text-ink-3 mb-3">
										Posición: <strong className="text-ink">{s.pos}°</strong> · {s.pts} pts
									</p>
		...
		<div className={`bg-surface-2 border border-line border-t-2 ${topBorder} rounded-2xl p-5`}>
			...
			<div className="flex gap-1 mb-3">
				...
				{team.last5.map((r, i) => (
					<span
						key={i}
						className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold text-white ${resultColor(r)}`}
					>
						{r === "W" ? "G" : r === "D" ? "E" : "P"}
					</span>
				))}
          ...
          ${team.attackRank <= 2 ? "bg-green-950 text-green-400 border-green-900" : "bg-yellow-950 text-yellow-400 border-yellow-900"}`}
				>
					⚔️ {team.attackRank <= 2 ? "Ataque fuerte" : "Ataque regular"} ({team.attackRank}/
					{team.totalTeams})
				</span>
				<span
					className={`text-xs px-2 py-0.5 rounded-full font-medium border
          ${team.defenseRank <= 2 ? "bg-green-950 text-green-400 border-green-900" : "bg-yellow-950 text-yellow-400 border-yellow-900"}`}
				>
					🛡️ {team.defenseRank <= 2 ? "Defensa sólida" : "Defensa regular"} ({team.defenseRank}/
					{team.totalTeams})
				</span>
			</div>
		...
		<div className="bg-pitch border-t border-line px-5 py-12">
			<div className="max-w-lg mx-auto text-center space-y-5">
				...
				<p className="text-ink-2 text-sm leading-relaxed max-w-sm mx-auto">
					...
					Live. Setup en menos de una semana.
				</p>
