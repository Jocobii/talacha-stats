  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div className="flex items-center gap-2 shrink-0">
        ...
        <ChevronRight size={16} strokeWidth={2} className="text-ink-3 group-hover:text-brand transition-colors" />
      </div>
    </div>
  );
}
...
function LeaderRow({ leader }: { leader: NonNullable<LeagueSnapshot["leader"]> }) {
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="text-xs font-semibold text-ink truncate flex-1">{titleCase(leader.teamName)}</span>
