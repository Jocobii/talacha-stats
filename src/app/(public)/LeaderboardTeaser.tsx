...
function Row({
  rank, name, alias, team, goals, delay, visible,
}: {
  rank: number; name: string; alias: string | null; team: string;
  goals: number; delay: number; visible: boolean;
}) {
  const isTop3 = rank <= 3;
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm font-semibold text-ink leading-tight truncate">
          {name}
          {alias && (
            <span className="text-brand font-normal ml-1.5">&quot;{alias}&quot;</span>
          )}
        </p>
