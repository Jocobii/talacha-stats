  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex gap-1 bg-surface border border-line rounded-xl p-1">
        <TabButton label="Temporada" active={active === "temporada"} onClick={() => setActive("temporada")} />
        <TabButton label="Carrera"   active={active === "carrera"}   onClick={() => setActive("carrera")}   />
      </div>

      {/* Tab content */}
      {active === "temporada" && (
        <TemporadaTab leagues={leagues} shareByLeague={shareByLeague} />
      )}
      {active === "carrera" && (
        <CarreraTab leagues={leagues} global={g} />
      )}
    </div>
  );
}
