		...
		const t = setTimeout(async () => {
			...
			try {
				const res = await fetch(
					`/api/players/search?q=${encodeURIComponent(query)}&mode=disambiguation`,
				);
				const data = await res.json();
				...
				<div>
					...
					{player.participations[0] && (
						<p className="text-xs text-ink-2 mt-2 ml-10">
							{player.participations[0].teamName} · {player.participations[0].leagueName} ·{" "}
							{player.participations[0].season}
						</p>
					)}

					{/* Extra participations */}
					{expanded &&
						player.participations.slice(1).map((p, i) => (
