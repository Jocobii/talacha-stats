      ...
      <header className="relative px-5 pt-8 pb-0 max-w-lg mx-auto w-full overflow-hidden">
        ...
        <div className="absolute inset-0 pointer-events-none" aria-hidden="true">
          <svg className="absolute inset-0 w-full h-full" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid slice">
            <circle cx="88%" cy="55%" r="70" fill="none" stroke="#00E676" strokeWidth="1" opacity="0.09" />
            <circle cx="88%" cy="55%" r="35" fill="none" stroke="#00E676" strokeWidth="0.8" opacity="0.06" />
            <circle cx="88%" cy="55%" r="5" fill="#00E676" opacity="0.12" />
            <line x1="0" y1="55%" x2="100%" y2="55%" stroke="#00E676" strokeWidth="0.6" opacity="0.04" />
          </svg>
          <div
            style={{
              position: "absolute",
              top: "-30%", right: "-15%",
              width: "55%", height: "160%",
              background: "radial-gradient(ellipse at center, rgba(0,230,118,0.08) 0%, transparent 65%)",
            }}
          />
        </div>
      ...
      <div className="flex-1 bg-surface rounded-t-3xl px-4 pt-5 pb-16">
        <div className="max-w-lg mx-auto">
          ...
          <div className="flex gap-1 bg-surface-2 border border-line p-1 rounded-xl mb-4">
            <ScopeTab href={cityTabHref} active={scope === "city"}>Ciudad</ScopeTab>
            <ScopeTab href={leagueTabHref} active={scope === "league"}>Liga</ScopeTab>
            <ScopeTab href={globalTabHref} active={scope === "global"}>Nacional</ScopeTab>
          </div>
...
function PodiumCard({ entry, pos }: { entry: RankingEntry; pos: 1 | 2 | 3 }) {
  ...
  return (
    ...
    >
      {/* Label de posición */}
      <span className={`inline-block text-[9px] font-bold uppercase tracking-widest border rounded-full px-2 py-0.5 mb-2 ${cfg.labelClass}`}>
        {cfg.label}
      </span>

      {/* Número de posición — protagonista */}
      <span className={`font-display font-black leading-none ${cfg.rankSize}`}>
        {pos}
      </span>
      ...
      {/* Avatar */}
      <div className={`w-11 h-11 rounded-full flex items-center justify-center font-display font-black text-lg mb-2 shrink-0 ${cfg.avatarClass}`}>
...
function DeltaBadge({ delta, isNew }: { delta: number | null; isNew: boolean }) {
  if (isNew) {
    return (
      <span className="text-[10px] font-bold text-brand uppercase tracking-wide">NEW</span>
    );
  }
...
function ScopeTab({ href, active, children }: { href: string; active: boolean; children: React.ReactNode }) {
  return (
    <Link
      href={href}
      className={`flex-1 text-center text-xs font-semibold py-2 rounded-lg transition ${active ? "bg-brand text-pitch shadow-sm" : "text-ink-3 hover:text-ink"
        }`}
    >
      {children}
    </Link>
...
function RankRow({
  ...
  return (
    <Link
      href={`/player/${entry.playerId}`}
      className={`flex items-center gap-3 rounded-2xl px-4 py-3.5 transition ${isTop10
    ...
    >
      ...
      <div className="text-right shrink-0 flex flex-col items-end gap-0.5">
        <p className={`font-display font-black text-2xl leading-none ${isTop10 ? "text-brand" : "text-ink"}`}>
          {entry.totalGoals}
        </p>
