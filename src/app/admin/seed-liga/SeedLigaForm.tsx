				...
				{!result ? (
					<form onSubmit={handleSubmit} className="space-y-6">
						...
						<section className="bg-gray-900 border border-gray-800 rounded-2xl p-6 space-y-4">
							<h2 className="text-sm font-semibold text-gray-300 uppercase tracking-wider">Liga</h2>
						...
						{loading && (
							<div className="text-center">
								<div className="inline-flex flex-col items-center gap-3">
									...
									<p className="text-xs text-gray-400">Simulando partidos y distribuyendo goles…</p>
								</div>
							</div>
						)}
					</form>
										...
										{result.standings.map((row) => (
											<tr key={row.name} className="hover:bg-gray-800/40">
												...
												<td className="px-3 py-2.5 text-center">
													{row.zone ? (
														<span
															className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${ZONE_STYLES[row.zone] ?? "bg-gray-700 text-gray-300"}`}
														>
															{row.zone}
														</span>
													) : (
