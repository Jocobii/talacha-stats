...
export default async function MatchPage({ params }: { params: Promise<{ id: string }> }) {
	...
	const homeEvents = match.events.filter(
		(e: { team: { id: string } }) => e.team.id === match.homeTeam.id,
	);
	const awayEvents = match.events.filter(
		(e: { team: { id: string } }) => e.team.id === match.awayTeam.id,
	);
	...
	return (
		<div>
			...
			<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
				<EventList title={match.homeTeam.name} events={homeEvents} />
				<EventList title={match.awayTeam.name} events={awayEvents} />
			</div>
...
function EventList({
	...
	events: {
		...
		eventType: string;
		minute: number | null;
		player: { fullName: string; alias: string | null };
	}[];
}) {
	return (
