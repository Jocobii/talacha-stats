...
export default async function MatchPreviewPage({ params }: { params: Promise<{ id: string }> }) {
	...
	const { match, teamForm, winProbability, topThreats, cardRisk, headToHead, narratorBullets } =
		preview;
	...
	return (
		<div className="max-w-4xl mx-auto">
			...
			<div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
				...
				/>
				<H2HCard homeTeam={match.homeTeam} awayTeam={match.awayTeam} h2h={headToHead} />
			</div>
		</div>
	);
}
			...
			<div className="flex items-center gap-2">
				...
				<div className="flex gap-1 ml-auto">
					{form.last5.map((r, i) => (
						<span
							key={i}
							className={`w-5 h-5 rounded-full ${resultColor(r)} text-white text-xs flex items-center justify-center font-bold`}
						>
							{r}
						</span>
					))}
				...
				<ul className="space-y-3">
					{threats.map((t) => (
						<li key={t.playerId} className="flex items-start justify-between gap-2">
							...
							<span
								className={`text-xs font-bold px-2 py-0.5 rounded-full whitespace-nowrap ${dangerColor(t.dangerRating)}`}
							>
								{t.dangerRating}
							</span>
						</li>
