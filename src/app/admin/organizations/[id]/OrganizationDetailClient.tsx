...
export default function OrganizationDetailClient({
  ...
  async function handleMemberAction(userId: string, action: "add" | "remove") {
    ...
    try {
      ...
      if (!data.ok) { setMemberError(data.error); return; }
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="lg:col-span-2">
        ...
        {org.leagues.length === 0 ? (
          <div className="bg-white rounded-xl shadow p-8 text-center">
            ...
            <Link href="/admin/leagues/new" className="text-green-600 text-sm hover:underline mt-2 block">
              Crear primera liga →
            </Link>
          </div>
        ) : (
      ...
      <div>
        ...
        <div className="bg-white rounded-xl shadow p-4 space-y-4">
          ...
          {isOwner && availableUsers.length > 0 && (
            <div className="border-t border-gray-100 pt-3">
              ...
              <div className="flex gap-2">
                ...
                <button
                  onClick={() => { if (selectedUser) handleMemberAction(selectedUser, "add"); }}
          ...
          {memberError && (
            <p className="text-red-600 text-xs">{memberError}</p>
          )}
        </div>
      </div>
    </div>
