...
export default function OrganizationSection({ leagueId, current, organizations, isOwner }: Props) {
	...
	return (
		<div className="bg-surface rounded-lg shadow p-4">
			<h2 className="text-sm font-semibold text-ink mb-3">Organización</h2>
				...
				<div className="space-y-3">
					...
					<div className="flex gap-2">
						...
						<button
							...
							className="bg-surface-2 text-ink-2 px-3 py-1.5 rounded-lg text-xs hover:bg-surface-2"
						>
							Cancelar
						</button>
					</div>
				...
				<div className="flex items-start justify-between gap-2">
					<div>
						{current ? (
									...
									<div className="mt-2 space-y-1">
										{current.members.map((m) => (
											<div key={m.id} className="flex items-center gap-1.5">
												...
												<p className="text-xs text-ink-2">{m.name}</p>
											</div>
										))}
									</div>
								)}
							...
							<p className="text-sm text-ink-3">Sin organización asignada</p>
						)}
					</div>
					{isOwner && (
						<button
